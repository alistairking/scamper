...
static void do_ping_handle_dl(scamper_task_t *task, scamper_dl_rec_t *dl)
{
  ...
  if(direction == 0)
	...
	goto err;
      ...
      reply->flags      |= SCAMPER_PING_REPLY_FLAG_PROBE_DLRX;
      if(probe->dlts != 0)
	reply->flags    |= SCAMPER_PING_REPLY_FLAG_PROBE_DLTX;

      if(state->sports == NULL)
	reply->probe_sport = ping->probe_sport;
      else
...
static void ping_handle_rt(scamper_route_t *rt)
{
  ...
  /*
   * if we're doing tcp ping, or relaying probes via a specific
   * router, or sending packet-too-big messages, determine the
...
static int ping_state_alloc(scamper_task_t *task)
{
  ...
  if((ping->flags & SCAMPER_PING_FLAG_RAW) != 0)
					  ...
					  scamper_cycle_t *cycle)
{
  ...
  if(SCAMPER_PING_METHOD_IS_TCP(ping))
    ...
    {
      if(SCAMPER_ADDR_TYPE_IS_IPV4(ping->dst))
	state->fds[0] = scamper_fd_udp4_dst(ping->src->addr, ping->probe_sport,
					    ping->dst->addr, ping->probe_dport);
      else
  ...
  for(i=1; i<state->fdc; i++)
	...
	state->fds[i] = scamper_fd_tcp6_dst(NULL, 0, state->sports, i,
					    ping->dst->addr, ping->probe_dport);
      if(state->fds[i] == NULL ||
	 scamper_fd_sport(state->fds[i], &state->sports[i]) != 0)
	goto err;
